 ...
 * @param vec 从小到大排序的数组
 * @param left 分段数组的最左下标，所以是int
 * @param right 分段数组的最右下标，所以是int
...
/**
 * @brief 删除所有重复的节点，而不是只删除一个
 * @param v 被删除的节点的值
 * @return 如果删除成功，返回true；如果一开始就没进行删除，那么就返回false
*/
template<class T>
 ...
 *				每次删除，当删除节点和其右子树的最左节点进行交换之后，从被删除的节点的父结点开始向上平衡；
